...
namespace APP_HOATHO.ViewModels.DuyetChungTu
{
    ...
    public class MoLaiChungTuDaDuyet_Header_ViewModel : BaseViewModel
    {
        #region "Field"
        ...
        private async void OnLoadExcute(object obj)
        {
            try
            {
                ...
                if (_documentType == DocumentType.MoLaiLCP_FOB)
                    url = $"api/DuyetChungTu/getMoLaiLCP_FOB?username={Preferences.Get(Config.User, "")}&fromdate={string.Format("{0:yyyy-MM-dd}", FromDate)}&todate={string.Format("{0:yyyy-MM-dd}", ToDate)}";
                else if (_documentType == DocumentType.MoLaiLCP_GC)
                    url = $"api/DuyetChungTu/getMoLaiLCP_GC?username={Preferences.Get(Config.User, "")}&fromdate={string.Format("{0:yyyy-MM-dd}", FromDate)}&todate={string.Format("{0:yyyy-MM-dd}", ToDate)}";
                else if (_documentType == DocumentType.MoLaiDatMua)
                    url = $"api/DuyetChungTu/getMoLaiDonDatMua?username={Preferences.Get(Config.User, "")}&fromdate={string.Format("{0:yyyy-MM-dd}", FromDate)}&todate={string.Format("{0:yyyy-MM-dd}", ToDate)}";
                ListItem.Clear();
                var a = await RunHttpClientGet<DuyetChungTuModel>(url);
                ListItem = a.Lists;               
