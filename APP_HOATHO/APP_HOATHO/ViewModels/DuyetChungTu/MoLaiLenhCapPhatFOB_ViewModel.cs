    ...
    public class MoLaiLenhCapPhatFOB_ViewModel : BaseViewModel
    {
        ...
        private async void OnLoadExcute(object obj)
        {
            try
            {
                ...
                ListItem.Clear();               
                var a = await RunHttpClientGet<DuyetChungTuModel>($"api/DuyetChungTu/getMoLaiLCP_FOB?username={Preferences.Get(Config.User, "")}&fromdate={string.Format("{0:yyyy-MM-dd}", FromDate)}&todate={string.Format("{0:yyyy-MM-dd}", ToDate)}");
                ListItem = a.Lists;                
                HideLoading();
            }
            catch (Exception ex)
