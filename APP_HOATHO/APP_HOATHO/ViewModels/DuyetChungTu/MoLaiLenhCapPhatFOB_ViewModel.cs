...
namespace APP_HOATHO.ViewModels.DuyetChungTu
{
    ...
    public class MoLaiLenhCapPhatFOB_ViewModel : BaseViewModel
    {
        #region "Field"
        ...
        public MoLaiLenhCapPhatFOB_ViewModel(DocumentType type)
        {
        ...
        private async void OnLoadExcute(object obj)
        {
            try
            {
                ...
                ListItem.Clear();
                var _json = Config.client.GetStringAsync(Config.URL + $"api/DuyetChungTu/getMoLaiLCP_FOB?username={Preferences.Get(Config.User, "")}&fromdate={string.Format("{0:yyyy-MM-dd}", FromDate)}&todate={string.Format("{0:yyyy-MM-dd}", ToDate)}").Result;
