...
namespace APP_HOATHO.ViewModels.DuyetChungTu
{
    ...
    public class DuyetLenhCapPhatGiaCong_ViewModel : BaseViewModel
    {
        #region "Field"
        ...
        public DuyetLenhCapPhatGiaCong_ViewModel(DocumentType type)
        {
            ...
            DuyetTatCaCommand = new Command(async () =>
            {
                try
                {
                    ...
                    if (ok == DialogReturn.OK)
                    {
                        ...
                        var result = await RunHttpClientPost("api/DuyetChungTu/DuyetTatCaLCP_GC", ListItem.ToList());
        ...
        private async void OnLoadExcute(object obj)
        {
            try
            {
                ...
                ListItem.Clear();
                var a = await RunHttpClientGet<DuyetChungTuModel>($"api/DuyetChungTu/getLenhCapPhat_GC?username={Preferences.Get(Config.User, "")}");
                ListItem = a.Lists;                
                HideLoading();
            }
            catch (Exception ex)
