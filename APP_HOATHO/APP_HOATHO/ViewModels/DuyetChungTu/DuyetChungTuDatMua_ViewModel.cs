       ...
       public DocumentType _documentType { get; set; }      
            ...
            DuyetTatCaCommand = new Command(async() =>
            {
                try
                {
                    ...
                    if (ok == DialogReturn.OK)
                    {
                        ...
                        var result = await RunHttpClientPost("api/DuyetChungTu/DuyetTatCaDonDatMua", ListItem.ToList());
        ...
        private async void OnLoadExcute(object obj)
        {
            try
            {
                ...
                ListItem.Clear();
                var a = await RunHttpClientGet<DuyetChungTuModel>($"api/DuyetChungTu/getDonDatMua?username={Preferences.Get(Config.User, "")}");
                ListItem = a.Lists;
                HideLoading();
            }
            catch (Exception ex)
