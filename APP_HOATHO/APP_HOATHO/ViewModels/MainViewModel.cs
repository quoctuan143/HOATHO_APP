...
namespace APP_HOATHO.ViewModels
{
    public class MainViewModel : BaseViewModel
    {
        ...
        private async void OpenDanhMucThietBi(object obj)
        {
            await new MessageYesNo("Bạn có muốn thoát chương trình không?").Show();
        }
        async Task Load()
        {
                ...
                await Device.InvokeOnMainThreadAsync(async () =>
                {
                    try
                    {
                        ...
                        if (_json.Contains("Không Tìm Thấy Dữ Liệu") == false && _json.Contains("[]") == false)
                        {
                            ...
                            if (IsDanhMucThietBi)
                            {
                               string url = "api/qltb/getThietBi?nhamay=" + Preferences.Get(Config.NhaMay, "");
